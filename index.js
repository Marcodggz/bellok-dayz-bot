...
function getFileState(st, filePath){ return st[filePath] || { size: 0, carry: '' }; }
function setFileState(st, filePath, obj){ st[filePath] = obj; saveState(st); }

// ================== ESTADO HEATMAP ==================
function loadHeat(){ return loadJSON(HEAT_STATE_FILE, { points: [], lastSentCount: 0 }); }
function saveHeat(h){ saveJSON(HEAT_STATE_FILE, h); }
...
async function nitDownload(filePath){
  try{
    ...
    else if (status===429 || looksLikeRateLimit(txt)) console.warn('[download] Rate-limit Nitrado.');
...
async function listAdmNames(dir, force=false){
  ...
  if (!force){
    ...
    if (now - lastRotateCheck < ROTATE_CHECK_MS) return listCache.get(dir) || [];
    lastRotateCheck = now;
  }
  ...
  for (let i=0;i<3;i++){
    try{
      ...
      const rows = entries
        ...
        .map(e=>({ name:e.name, path: e.path || `${dir.replace(/\/+$/,'')}/${e.name}` }))
      ...
      if (html || rl){
        if (i===2){ listCooldownUntil = Date.now() + LIST_COOLDOWN_MS; console.warn('[list] cooldown', Math.round(LIST_COOLDOWN_MS/1000),'s'); return listCache.get(dir) || []; }
        const wait = 800 * Math.pow(2,i); if (DEBUG) console.log(`[list] backoff ${wait}ms`); await new Promise(r=>setTimeout(r, wait));
      } else {
...
async function ensureLatestAdmSelected(){
  if (!ADM_DIR){ console.warn('[rotate] Falta NITRADO_ADM_DIR'); return; }
  const rows = await listAdmNames(ADM_DIR, !INITIALIZED /*force*/);
  if (!rows.length) { if (DEBUG) console.log('[rotate] sin rows en listado'); return; }
  ...
  if (latest !== CURRENT_ADM){
    ...
    if (HEATMAP_RESET_ON_ROTATE){
      const h = loadHeat(); h.points = []; h.lastSentCount = 0; saveHeat(h);
    }
  }
}
...
async function readNewLines(filePath){
  ...
  if (fsState.carry){ chunk = fsState.carry + chunk; fsState.carry = ''; }

  if (chunk && !/\r?\n$/.test(chunk)){
    const lastNL = chunk.lastIndexOf('\n');
    if (lastNL===-1){ fsState.carry = chunk; chunk=''; }
    else { fsState.carry = chunk.slice(lastNL+1); chunk = chunk.slice(0,lastNL+1); }
  }
...
function cleanWeapon(s){ if (!s) return null; return String(s).replace(/\s+from.*$/i,'').replace(/\s+at.*$/i,'').replace(/\s*\(.*?\)\s*$/,'').trim(); }
function cleanDevice(s){ if (!s) return null; return String(s).replace(/^an?\s+/i,'').replace(/\s+from.*$/i,'').replace(/\s*\(.*?\)\s*$/,'').trim(); }
...
function parseKill(line){
  ...
  for (const re of PVP_PATTERNS){
    ...
    if (m){
      ...
      if (/killed Player/i.test(re.source)) { killer = m[1]; victim = m[2]; weapon = cleanWeapon(m[3]); }
      else { victim = m[1]; killer = m[2]; weapon = cleanWeapon(m[3]); }
...
function updatePositionsFromLine(line){
  const re = /Player\s+["'“”]([^"'“”]+)["'“”]\s*\([^)]*?pos=<\s*([0-9.]+)\s*,\s*([0-9.]+)\s*,\s*([0-9.\-]+)\s*>\)/ig;
  let m;
  while ((m = re.exec(line)) !== null){
    const name = m[1]; const x = +m[2]; const y = +m[3];
    if (!Number.isNaN(x) && !Number.isNaN(y)) lastPosByName.set(name, {x,y, ts:Date.now()});
  }
}
function posForVictimFromLine(victim, line){
  const rex = new RegExp(`Player\\s+["'“”]${escapeRegExp(victim)}["'“”][^\\n]*?pos=<\\s*([0-9.]+)\\s*,\\s*([0-9.]+)\\s*,\\s*([0-9.\\-]+)\\s*>`, 'i');
...
// ================== DEDUPE 20s ==================
function typeRank(tp){ return tp==='pvp' ? 2 : tp==='explosion' ? 1 : 0; }
function timeToSec(t){ if (!t) return null; const [hh,mm,ss] = t.split(':').map(Number); if ([hh,mm,ss].some(Number.isNaN)) return null; return hh*3600+mm*60+ss; }
const BUCKET_S = 20;
function victimBucketKey(victim, t){ const s=timeToSec(t); const b = s==null ? Math.floor(Date.now()/1000/BUCKET_S) : Math.floor(s/BUCKET_S); return `${victim}|${b}`; }
const sentBuckets = new Map();
const SENT_TTL_MS = 60*60*1000;
function alreadySentBucket(key){ const now=Date.now(); for (const [kk,ts] of sentBuckets) if (now-ts>SENT_TTL_MS) sentBuckets.delete(kk); if (sentBuckets.has(key)) return true; sentBuckets.set(key, now); return false; }

// ================== EMBEDS ==================
function embedPvp({ killer, victim, weapon }){
  const txt = weapon ? `"${killer}" killed "${victim}" with "${weapon}"` : `"${killer}" killed "${victim}"`;
  return { embeds: [ new EmbedBuilder().setColor(0xE11D48).setDescription(txt).setTimestamp(new Date()) ] };
}
function embedExplosion({ victim, device }){
  const dev = device || 'explosive';
  const txt = `"${victim}" died from an "${dev}" explosion`;
  return { embeds: [ new EmbedBuilder().setColor(0xF59E0B).setDescription(txt).setTimestamp(new Date()) ] };
}
function buildKillEmbed(k){ if (k.type==='pvp') return embedPvp(k); if (k.type==='explosion') return embedExplosion(k); return null; }

// ================== HEATMAP RENDER (con calibración + mejoras) ==================
function renderHeatPng(points, outPath, baseMapPath = '') {
  let basePng = null;
  let W = HEATMAP_WIDTH, H = HEATMAP_HEIGHT;
  ...
  try {
    ...
    console.warn('[heatmap] no se pudo leer CHERNARUS_MAP_PATH, uso lienzo transparente:', e.message);
  }
  ...
  for (let dy = -radius; dy <= radius; dy++) {
    for (let dx = -radius; dx <= radius; dx++) {
      ...
      if (d2 <= radius * radius) ker.push({ dx, dy, w: Math.exp(-d2 / twoSigma2) });
    }
  }
  ...
  for (const p of hp) {
    ...
    const timeWeight = HEAT_HALFLIFE_MIN > 0 ? Math.pow(0.5, ageMin / HEAT_HALFLIFE_MIN) : 1;

    const { px, py } = mapToPixelCoords(p.x, p.y, W, H);
    for (const k of ker) {
      const x = px + k.dx, y = py + k.dy;
      if (x >= 0 && x < W && y >= 0 && y < H) inten[y * W + x] += k.w * timeWeight;
    }
  }
  ...
  if (maxv > 0) {
    ...
    if (HEAT_NORM_PERCENTILE > 0 && HEAT_NORM_PERCENTILE < 1) {
      ...
      const vals = [];
      for (let i = 0; i < inten.length; i += sampleStep) if (inten[i] > 0) vals.push(inten[i]);
      if (vals.length) {
        vals.sort((a,b)=>a-b);
        const idx = Math.max(0, Math.min(vals.length - 1, Math.floor(vals.length * HEAT_NORM_PERCENTILE) - 1));
    ...
    for (let i = 0; i < inten.length; i++) {
      ...
      const a = Math.round(clamp(HEAT_MIN_ALPHA + (255 - HEAT_MIN_ALPHA) * t, HEAT_MIN_ALPHA, 255));
  ...
  if (HEAT_RECENT_MIN > 0) {
    const R = HEAT_RECENT_DOT_RADIUS > 0 ? HEAT_RECENT_DOT_RADIUS : Math.max(4, Math.floor(Math.min(W, H) / 180));
    const A = clamp(HEAT_RECENT_DOT_ALPHA, 0, 255);
    const rr = R * R;
    const red = 225, green = 29, blue = 72; // #E11D48
  ...
  if (basePng) {
    ...
    for (let i = 0; i < W * H; i++) {
      const o  = i * 4;
      const br = basePng.data[o + 0], bg = basePng.data[o + 1], bb = basePng.data[o + 2], ba = basePng.data[o + 3] / 255;
      const or = overlay.data[o + 0], og = overlay.data[o + 1], ob = overlay.data[o + 2], oa = overlay.data[o + 3] / 255;
...
async function maybeSendHeatmap(client){
  ...
  try{
    ...
    if (!ch || typeof ch.send!=='function'){ console.warn('[heatmap] canal inválido'); return; }
    const file = new AttachmentBuilder(HEAT_IMG_PATH);
    await ch.send({ content: '⚔️PVP heat-map 🥵', files: [file] }); // único mensaje
    h.lastSentCount = h.points.length; saveHeat(h);
    lastHeatSentAt = now;
  }catch(e){
...
async function runBot(){
  ...
  async function tick(){
    try{
      ...
      if (!CURRENT_ADM) { if (DEBUG_TICKS) console.log('[tick] sin CURRENT_ADM'); return; }

      const lines = await readNewLines(CURRENT_ADM);
      if (DEBUG_TICKS) console.log(`[tick] ${new Date().toLocaleTimeString()}  +${lines.length} líneas nuevas`);
      if (!lines.length) { await maybeSendHeatmap(client); return; }

      for (const ln of lines) updatePositionsFromLine(ln);

      const events = [];
      for (const ln of lines){ const e = parseKill(ln); if (e) events.push(e); }
      ...
      if (!ch || typeof ch.send!=='function'){ console.warn('[tick] Missing Access / canal no válido.'); return; }
      ...
      for (const [key, k] of groups){
        ...
        try{
          ...
          if (RAW_TO_DISCORD){
            const raw = lines.find(l => l.includes(`"${k.victim}"`) && (k.t ? l.startsWith(k.t) : true));
            if (raw) await ch.send('```'+raw+'```');
          }
        }catch(e){ console.error('[send error]', e?.code || e?.message || e); }

        // sumar punto al heatmap con la última posición conocida de la víctima
        const pos = posForVictimFromLine(k.victim, k.line || '');
        if (pos && Number.isFinite(pos.x) && Number.isFinite(pos.y)) addHeatPoint(pos.x, pos.y);
      }
      ...
      if (looksLikeHtml(txt) || status===429 || looksLikeRateLimit(txt)) {
        if (Date.now() >= listCooldownUntil) console.warn('[tick] Nitrado ocupado; cooldown.');
...
async function runDiscordTest(){
  ...
  client.once('ready', async ()=>{
    try{
      ...
      await ch.send({ embeds: [ new EmbedBuilder().setColor(0x22C55E).setDescription('✅ Prueba: el bot puede enviar mensajes aquí.').setTimestamp(new Date()) ] });
...
async function runDiscordHeatmapTest(){
  checkEnv();
  if (!HEATMAP_CHANNEL_ID){ console.error('Falta HEATMAP_CHANNEL_ID en .env'); process.exit(1); }
  const client = new Client({ intents: [GatewayIntentBits.Guilds] });
  client.once('ready', async ()=>{
    try{
      const ch = await client.channels.fetch(HEATMAP_CHANNEL_ID);
      await ch.send({ embeds: [ new EmbedBuilder().setColor(0x3B82F6).setDescription('🧪 Prueba: canal de heat-map OK').setTimestamp(new Date()) ] });
...
async function runDiagnose(){
  ...
  if (!rows.length){
    console.log('[diagnose] ❌ No se listan .ADM (rate-limit o ruta incorrecta).');
    process.exit(1);
  }
  ...
  if (dl.error){ console.log('[diagnose] ❌ No se pudo descargar el ADM'); process.exit(1); }
  ...
  for (const ln of tail){
    ...
    if (e){ if (e.type==='pvp') pvp++; else if (e.type==='explosion') exp++; }
  }
